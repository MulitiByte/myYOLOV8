...
def val(cfg=DEFAULT_CFG, use_python=False):
    ...
    model=r"D:\Internship_C\about-yolov8\ultralytics-main\results\612Seg_Model\weights\best.pt"
    # data = cfg.data or 'coco128-seg.yaml'
    data=r'D:\Internship_C\about-yolov8\ultralytics-main\ultralytics\yolo\cfg\cityspaces.yaml'
    ...
    res_list = ['all','roads','sidewalk','building','wall','fence','pole','traffic light','traffic sign','vegetation','terrain','sky','person'
,'rider','car','truck','bus','train','motorcycle','bicycle']
    res_dict = dict()
    for index,i in enumerate(res_list):
        if i=='all':
            continue
        res_dict[i] = a.metrics.box.f1[index-1]
    res_dict['all']=np.mean(a.metrics.box.f1)
    print("                    f1:")
    for i in list(res_dict.keys()):
        print(f"{i:>20}{res_dict[i]:>10.3f}")
    # return res_dict
