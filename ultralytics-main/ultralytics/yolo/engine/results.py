...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            #重构idx，选择类别输出分割对象，如果不需要的话进行1，2两个步骤
            #1.1 删除下面的代码
            new_idx=[]
            #wanted_seg_cls想要选择的类型列表
            wanted_seg_cls=[i for i in range(18) if i not in [2,10]]
            # wanted_seg_cls=[i for i in range(80)]
            chosen_mask_index=[]
            for i,cls in enumerate(idx):
                if int(cls) in wanted_seg_cls:
                    new_idx.append(int(cls))
                    chosen_mask_index.append(i)
            new_idx=torch.tensor(new_idx,dtype=torch.float32)
            idx=new_idx
            #1.2 删除上面的代码
            #2.把下面的pred_masks.data[chosen_mask_index]改成pred_masks.data
            if chosen_mask_index:
                annotator.masks(pred_masks.data[chosen_mask_index], colors=[colors(x, True) for x in idx], im_gpu=img_gpu)

        if pred_boxes and show_boxes:
            for d in reversed(pred_boxes):      
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(d.id.item())
                # 选择类别输出分割对象，如果不需要的话进行1，2两个步骤
                #1.修改wanted_detection_cls为自己想要的类别数
                wanted_detection_cls=[11,12,13,14,15,16,17,18]
                # wanted_detection_cls = [0,1,2]
                #2.删除251行，252-254缩进向前
                if c in wanted_detection_cls:
